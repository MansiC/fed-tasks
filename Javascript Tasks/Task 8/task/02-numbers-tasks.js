...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  let u = Math.sqrt(Math.pow(x1, 2) + Math.pow(y1, 2));
  let v = Math.sqrt(Math.pow(x2, 2) + Math.pow(y2, 2));
  let uv = x1 * x2 + y2 * y1;

  return Math.acos(uv / (u * v));
}
...
function roundToPowerOfTen(num, pow) {
  var x = num % Math.pow(10, pow);

  return x > 5 * Math.pow(10, pow - 1) ? num - x + Math.pow(10, pow) : num - x;
}
...
function toNumber(value, def) {
  if (isNaN(parseInt(value))) return def;
